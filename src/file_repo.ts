...
const _readFile: (
    ...
    options:
        ...
        | undefined
        | null,
) => Promise<string | Buffer> = Promise.promisify(readFile);
...
export class FileRepo implements deferred_repository<string> {
    ...
    create(options: resource_data<string>): Promise<boolean> {
        ...
        if (options.depends && !is_depends_object(options.depends))
            throw Error(
                `options.depends is not a valid depency object (${JSON.stringify(
                    options.depends,
                )}) . Check that names are valid and values are semver strings`,
            );
                ...
                if (latest_version) {
                    if (options.upsert) {
                        if (
                            ...
                            var err = new Error(
                                `Version (${loc.version}) preceeds the latest version (${latest_version})`,
                            );
                        ...
                        if (
                            ...
                            var err = new Error(
                                `Version (${loc.version}) does not exceed the latest version (${latest_version})`,
                            );
                            return Promise.reject(err);
                        }
                    }
    ...
    fetchOne(
        options: package_loc,
        opts?: fetch_opts,
    ): Promise<resource_data<string>> {
        var versionPromise: Promise<string>;
        if (options.version) {
            //console.log("VALIDING VERSION PARAMETER")
            versionPromise = this.versions(options.name).then(
                (versions: string[]) => {
                    ...
                    if (!version) {
                        throw new Error(
                            `No such version: "${
                                options.version
                            }". Valid versions include: ${JSON.stringify(
                                versions,
                            )}`,
                        );
                    }
        ...
        if (!!opts && opts.novalue) {
            ...
            return versionPromise.then((version: string) => {
                ...
                const value_path = this.get_path(
                    { name: options.name, version: version },
                    "value",
                );
                //console.log("const depends_path = this.get_path")
                const depends_path = this.get_path(
                    { name: options.name, version: version },
                    "depends",
                );
    ...
    fetch(
        query: package_loc | package_loc[],
        opts?: fetch_opts,
    ): Promise<resource_data<string>[]> {
        if (Array.isArray(query)) {
            const names = query.map(x => x.name);
            return this.depends(query).then(pkgs =>
                Promise.all(
                    pkgs
                        .filter(
                            x =>
            ...
            return this.depends([query]).then(pkgs =>
                Promise.all(pkgs.map(x => this.fetchOne(x, opts))),
            );
        } else {
    ...
    update(options: resource_data<string>): Promise<boolean> {
        if (this.config.update == "none") {
            return Promise.reject(
                new Error("updates are disableed in this repository"),
            );
        }
        ...
        var out;
        if (
            this.config.update === undefined ||
            this.config.update == "latest"
        ) {
            out = this.latest_version(loc.name).then(
                (latest_version: string) => {
                    if (semver.neq(latest_version, loc.version))
                        throw new Error(
                            "Only the most recent version of a package may be updated",
                        );
                    return true;
                },
            );
    ...
    del(options: package_loc): Promise<boolean> {
        ...
        return Promise.all([
            ...
            _unlink(this.get_path(loc, "depends")).catch(e => {
                /* pass */
            }),
        ])
            .then(() => true)
    ...
    depends(
        x: package_loc | package_loc[] | { [key: string]: string },
    ): Promise<package_loc[]> {
        var bare_repo: bare_deferred_readable_repository = {
            fetchOne: (request: package_loc, opts: fetch_opts) =>
                this.fetchOne(request, { novalue: true }),
        ...
        if (is_package_loc(x)) {
            ...
            if (!ajv_is_depends_object(x))
                throw Error(
                    `Expected an object with valid names and semver strings but got error ${ajv_is_depends_object.errors}`,
                );
            var y: package_loc[] = Object.keys(x)
    ...
    packages(): Promise<string[]> {
        var self = this;
        return _readDir(this.config.directory).then(function(
            pkg_names: string[],
        ) {
            return Promise.all(
                pkg_names.map(function(_name: string) {
                    return _stat(path.join(self.config.directory, _name)).then(
                        function(x: Stats): boolean | Promise<boolean> {
    ...
    versions(
        pkg?: string,
    ): Promise<{ [x: string]: string[] }> | Promise<string[]> {
        if (typeof pkg === "undefined") {
            ...
            return this.packages()
                .then(names => {
                    return Promise.all(
                        names.map(name => {
                            return self.versions(name).then(versions => {
                                out[name] = versions;
                                return true;
                            });
                        }),
                    ...
                    if (
                        ...
                        throw new Error(
                            "No such resource: " +
                                pkg +
                                " at path: " +
                                path.join(this.config.directory, pkg),
                        );
                    } else {
                        throw e;
                    }
                ...
                .then(function(file_names: string[]) {
                    ...
                    return Promise.all(
                        file_names.map(function(x) {
                            // RETURN ONLY FILES
                            return _stat(
                                path.join(self.config.directory, pkg, x),
                        ...
                        .then(mask => {
                            ...
                            for (var i = mask.length - 1; i >= 0; i--) {
                                if (!mask[i]) file_names.splice(i, 1);
                            }
    ...
    get_file_name(options: package_loc) {
        ...
        if (options.version) {
            return this.versions(options.name).then(function(versions) {
                ...
                if (!version) throw new Error("No compatibile version");
                return path.join(
                    options.name,
                    "v" + version + (self.config.ext ? self.config.ext : ""),
                );
            });
        }
        // no version provided
