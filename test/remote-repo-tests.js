...
var repo = require('../index'), https = require('https'), http = require('http'), express = require('express'), app = express();
...
//------------------------------------------------------------
// BUILD THE BASE URL
var server = ('function' === typeof app) ? http.createServer(app) : app, address = server.address();
if (!address) {
    server.listen(0);
...
describe('Remote Repo (temp/parser)', function () {
    ...
    it('POST(create) should require a version', function (done) {
        remote_repo.create({ name: 'foo' })
            .should.eventually.be.rejectedWith(Error, "missing required value options.version").and.notify(done);
    });
    it('POST(create) should create a library', function (done) {
        remote_repo.create({ name: 'foo', version: 'v1.1.1' }, 'My favorite equations')
            .should.eventually.equal(true).and.notify(done);
    });
    it('POST(create) should create a library', function (done) {
        // FOR TESTING THE READ ONLY BUFFER
        remote_repo.create({ name: 'bar', version: 'v1.1.1' }, 'My favorite equations')
            .should.eventually.equal(true).and.notify(done);
    });
    it('POST(create) should not create a library twice', function (done) {
        remote_repo.create({ name: 'foo', version: 'v1.1.1' }, 'My favorite equations')
            .should.eventually.be.rejectedWith(Error, "Version (1.1.1) does not exceed the latest version (1.1.1)").and.notify(done);
    });
    it('POST(create) should not create a previous version', function (done) {
        remote_repo.create({ name: 'foo', version: 'v1.1.0' }, 'My favorite equations')
            .should.eventually.be.rejectedWith(Error, "Version (1.1.0) does not exceed the latest version (1.1.1)").and.notify(done);
    });
    it('GET(fetch) should get the library', function (done) {
...
describe('Buffered Remote Repo (temp/parser)', function () {
    ...
    it('CREATE call should be rejected.', function (done) {
        buffered_repo.create({ name: 'foo', version: "1.1.1" }, "ASFD")
            .should.eventually.be.rejectedWith(Error, "Cannot create a record in a read-only buffer").and.notify(done);
    });
    it('UPDATE call should be rejected.', function (done) {
        buffered_repo.update({ name: 'foo', version: "1.1.1" }, "asdf")
            .should.eventually.be.rejectedWith(Error, "Cannot update a record in a read-only buffer").and.notify(done);
    });
    it('DELETE call should be rejected.', function (done) {
        buffered_repo.del({ name: 'foo', version: "1.1.1" })
            .should.eventually.be.rejectedWith(Error, "Cannot delete a record in a read-only buffer").and.notify(done);
    });
});
// A READ-ONLY BUFFERED REPO BASED WRAPPING THE FILE SYSTEM REPO
var proc_repo;
describe('======= Processed Repo =======', function () {
    it('should not throw an error', function (done) {
        proc_repo = new repo.ProcessedBuffer(remote_repo, function (x) { return "Processed: " + String(x); });
    ...
    // FETCH-sync
    it("fetch_sync({name:'bar'}) should throw when called before the item is defined (by fetching!)", function (done) {
        try {
    ...
    // FETCH-sync
    it("fetch_sync({name:'bar'}) should return the library", function () {
